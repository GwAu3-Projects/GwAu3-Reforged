...
namespace GW {
    ...
    bool RPCBridge::HandleRequest(const PipeRequest& request, PipeResponse& response) {
        ...
        try {
            ...
            case UNREGISTER_EVENT_BUFFER:
                ...
                // Array operations
            case READ_MEMORY_ARRAY:
                return HandleArrayRequest(request, response);

                // Batch operations
            case BATCH_READ_MEMORY:
                return HandleBatchReadRequest(request, response);

                // Heartbeat
            case HEARTBEAT:
                return HandleHeartbeatRequest(request, response);
    ...
    bool RPCBridge::HandleServerControlRequest(const PipeRequest& request, PipeResponse& response) {
        ...
        case SERVER_STATUS: {
            ...
            response.success = 1;
            response.server_status.status = server.IsRunning() ? 1 : 0;
            response.server_status.client_count = server.GetClientCount();
            response.server_status.uptime_ms = server.GetUptimeMs();
            strncpy_s(response.server_status.pipe_name,
                      sizeof(response.server_status.pipe_name),
                      server.GetPipeName().c_str(),
                      _TRUNCATE);
            LOG_INFO("Server status requested: %s, clients: %u, uptime: %llu ms",
                server.IsRunning() ? "Running" : "Stopped",
                response.server_status.client_count,
                response.server_status.uptime_ms);
            break;
        }
    ...
    bool RPCBridge::HandleDLLControlRequest(const PipeRequest& request, PipeResponse& response) {
        ...
        case DLL_STATUS: {
            ...
            response.dll_status.version = GW::DLL_VERSION;

            // Build info: "vX.Y.Z Type"
            snprintf(response.dll_status.build_info,
                     sizeof(response.dll_status.build_info),
                     "v%u.%u.%u %s",
                     GW::DLL_VERSION_MAJOR,
                     GW::DLL_VERSION_MINOR,
                     GW::DLL_VERSION_PATCH,
                     GW::DLL_BUILD_TYPE);

            LOG_INFO("DLL status requested: state=%d, version=0x%X, build=%s",
                response.dll_status.status,
                response.dll_status.version,
                response.dll_status.build_info);
            break;
        }
    ...
    bool RPCBridge::HandleArrayRequest(const PipeRequest& request, PipeResponse& response) {
        switch (request.type) {
        case READ_MEMORY_ARRAY: {
            uintptr_t address = request.array_read.address;
            uint8_t elementType = request.array_read.element_type;
            uint32_t elementCount = request.array_read.element_count;

            // Determine element size based on type
            uint32_t elementSize = 0;
            switch (elementType) {
            case PARAM_INT8:    elementSize = 1; break;
            case PARAM_INT16:   elementSize = 2; break;
            case PARAM_INT32:   elementSize = 4; break;
            case PARAM_INT64:   elementSize = 8; break;
            case PARAM_FLOAT:   elementSize = 4; break;
            case PARAM_DOUBLE:  elementSize = 8; break;
            case PARAM_POINTER: elementSize = sizeof(uintptr_t); break;
            default:
                strcpy_s(response.error_message, "Invalid element type");
                response.success = 0;
                return true;
            }
            ...
            if (totalSize > sizeof(response.array_result.data)) {
                strcpy_s(response.error_message, "Array too large (max 2048 bytes)");
                response.success = 0;
                return true;
            }
            ...
            if (!address) {
                strcpy_s(response.error_message, "Invalid address (null)");
                response.success = 0;
                return true;
            }
            ...
            __except (EXCEPTION_EXECUTE_HANDLER) {
                strcpy_s(response.error_message, "Memory read exception");
                response.success = 0;
                return true;
            }

            // Fill response
            response.array_result.element_type = elementType;
            response.array_result.element_count = elementCount;
            response.array_result.element_size = elementSize;
            response.array_result.total_size = totalSize;
            response.success = 1;

            LOG_INFO("Read array: addr=0x%X, type=%d, count=%u, size=%u bytes",
                address, elementType, elementCount, totalSize);

            return true;
        }

        default:
            strcpy_s(response.error_message, "Unknown array request type");
            response.success = 0;
            return false;
        }
    }

    bool RPCBridge::HandleBatchReadRequest(const PipeRequest& request, PipeResponse& response) {
        uint8_t count = request.batch_read.count;

        // Validate count
        if (count == 0 || count > 32) {
            strcpy_s(response.error_message, "Invalid batch count (must be 1-32)");
            response.success = 0;
            return true;
        }

        response.batch_result.count = count;
        memset(response.batch_result.success_mask, 0, sizeof(response.batch_result.success_mask));
        memset(response.batch_result.values, 0, sizeof(response.batch_result.values));

        uint8_t successCount = 0;

        for (uint8_t i = 0; i < count; i++) {
            uintptr_t addr = request.batch_read.addresses[i];
            uint8_t size = request.batch_read.sizes[i];
            ...
            __try {
                uint64_t value = 0;
                switch (size) {
                case 1: value = *reinterpret_cast<uint8_t*>(addr); break;
                case 2: value = *reinterpret_cast<uint16_t*>(addr); break;
                case 4: value = *reinterpret_cast<uint32_t*>(addr); break;
                case 8: value = *reinterpret_cast<uint64_t*>(addr); break;
                }

                response.batch_result.values[i] = value;

                // Set success bit
                response.batch_result.success_mask[i / 8] |= (1 << (i % 8));
                successCount++;
            }
            __except (EXCEPTION_EXECUTE_HANDLER) {
                // Read failed, bit stays 0
            }
        }

        response.success = (successCount > 0) ? 1 : 0;

        LOG_INFO("Batch read: %u/%u addresses read successfully", successCount, count);

        return true;
    }

    bool RPCBridge::HandleHeartbeatRequest(const PipeRequest& request, PipeResponse& response) {
        uint32_t clientTimestamp = request.heartbeat.client_timestamp;
        uint32_t serverTimestamp = GetTickCount();
        ...
        LOG_DEBUG("Heartbeat: client=%u, server=%u, latency=%u ms",
            clientTimestamp, serverTimestamp, response.heartbeat_result.latency_ms);
