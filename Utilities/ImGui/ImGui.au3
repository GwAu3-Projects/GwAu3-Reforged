...
; Try FileInstall first (works when compiled), then fallback to direct file copy
If Not FileInstall("..\..\Utilities\ImGui\imgui.dll", $__imgui_dllpath) Then
	; Not compiled - try multiple possible locations
	Local $__imgui_dllsource = ""
	Local $__imgui_paths[3] = [ _
		@ScriptDir & "\..\..\Utilities\ImGui\imgui.dll", _
		@ScriptDir & "\Utilities\ImGui\imgui.dll", _
		StringRegExpReplace(@ScriptFullPath, "\\[^\\]+$", "") & "\..\..\Utilities\ImGui\imgui.dll" _
	]

	For $__i = 0 To UBound($__imgui_paths) - 1
		If FileExists($__imgui_paths[$__i]) Then
			$__imgui_dllsource = $__imgui_paths[$__i]
			ExitLoop
		EndIf
	Next

	If $__imgui_dllsource <> "" Then
		FileCopy($__imgui_dllsource, $__imgui_dllpath, 1)
	Else
		MsgBox(16, "Error", "Cannot find imgui.dll" & @CRLF & "Searched in:" & @CRLF & _
			$__imgui_paths[0] & @CRLF & $__imgui_paths[1] & @CRLF & $__imgui_paths[2])
		Exit
	EndIf
EndIf
